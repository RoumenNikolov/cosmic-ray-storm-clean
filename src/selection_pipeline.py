...
Orchestrates two independent feature selection methods (LASSO, RFECV)
across all forecast horizons and consolidates the results via
majority vote.

Classes
-------
FeatureSelectionPipeline
    Runs LassoSelector and RFECVSelector for each horizon in K_HORIZONS.
    A feature is included in SELECTED_FEATURES if it survives in at
    least min_votes methods at any horizon (union across horizons).

    Two modes are supported:
    - min_votes=2 (intersection): feature must survive both LASSO and RFECV
    - min_votes=1 (union): feature survives if in at least one method

    The cross-referencing approach is consistent with Vasilev (2023)
    who combined multiple independent methods and removed only features
    eliminated by all methods.
...
RFECV uses TimeSeriesSplit internally — temporally correct for lag features.
SHAPSelector is retained in feature_selector.py for future use in the
modelling notebook where computational resources are less constrained.

Mutual Information was excluded because RFECV on LinearRegression already
captures the relevant signal with temporal correctness.
"""

import numpy as np
import pandas as pd

from .feature_selector import LassoSelector, RFECVSelector
from .utils import selection_summary


class FeatureSelectionPipeline:
    """
    Orchestrates LASSO and RFECV feature selection across all horizons.
    ...
    rfecv_splits : int — TimeSeriesSplit folds for RFECV (default 5)
    ...
    def __init__(
        ...
        self.rfecv_splits = rfecv_splits
        self.min_votes    = min_votes
        self.random_state = random_state

        self.results_     = {'lasso': {}, 'rfecv': {}}
        self.summary_     = None
        self.selected_    = None
    ...
    def fit(self, X: pd.DataFrame, feat: pd.DataFrame) -> "FeatureSelectionPipeline":
        """
        Run LASSO and RFECV selectors for each horizon.
        ...
        for k in self.k_horizons:
            ...
            rfecv = RFECVSelector(
                feature_cols = self.feature_cols,
                storm_thr    = self.storm_thr,
                n_splits     = self.rfecv_splits,
            ).fit(X, y)
            self.results_['rfecv'][k] = rfecv.get_selected()
            print(f"        retained={len(rfecv.get_selected())}/{len(self.feature_cols)}")
    ...
    def print_summary(self):
        ...
        print("─" * 55)
        for feat, votes in self.vote_counts_.sort_values(ascending=False).items():
            selected = "✓" if feat in self.selected_ else "✗"
            print(f"{feat:>30} {int(votes):>10} {selected:>10}")
